    ...
    public partial class CODE
    {
        ...
        public class BINDING
        {
            ...
            public void test()
            {
                ...
                var element = new {position =  new {x = 10, y = 0.1}, variable = variable, nodes = add(
                    new object[]{
                            timeline,
                            obj.position, 800,
                            obj.rotation, 801,
            ...
            int propIdKey;
            int increment = 0;
            public object add(object[] options)
            {
                object[] instructions = code.instructionSet(options);
                ...
                for (int t = 0; t < timelineCount; t++) {
                    ...
                    for (int o = 0; o < objCount; o++) {
                        ...
                        float dataTypePrecision = (float?)instructions[5] ?? (float)Math.Type.precision(dataType);
                        ...
                        if (param is TLPoly)
                        {
                            props = Math.Poly.generate(dataType, (float[])param.poly, (float[])initPropValue, dataTypePrecision);
                            pkeys = props.Length == paramKeys.Count() ? paramKeys : Math.Poly.generateKeys(props, paramKey ?? propIdKey);
                        }
                        else
                        {
                            props = Math.Type.convertToPrecisionDataType(dataType, (object[])propValues, 1, 2, dataTypePrecision);
                            pkeys = props.Length == paramKeys.Count() ? paramKeys : Math.Poly.generateKeys(props, paramKey ?? propIdKey);
                        }
            ...
            public object queue(object[] options)
            {
                object[] instructions = code.instructionSet(options);
